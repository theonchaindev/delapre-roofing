    ...
    <div className="page-enter">
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-semibold text-neutral-900 group-hover:text-gold-600 transition-colors">{p.name}</h3>
                      {p.duration && (
                        <span className="text-xs text-neutral-400 border border-neutral-200 px-2 py-0.5 rounded-full">
                          {p.duration}
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-neutral-500 leading-relaxed mb-4 line-clamp-2 mt-1">{p.desc}</p>
                    <div className="flex flex-wrap gap-2 mb-4">
                      {p.services.map((srv) => (
                        <span
                          key={srv}
                          className="text-xs text-gold-600 bg-gold-500/5 border border-gold-400/20 px-2.5 py-1 rounded-full"
                        >
                          {srv}
                        </span>
                      ))}
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-gold-600 font-semibold group-hover:gap-2.5 transition-all">
                      View project
      ...
      <section className="py-20 bg-dark-900">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <Reveal>
            <div>
              <h2 className="text-3xl font-bold text-white tracking-tight mb-2">Want to see your project here?</h2>
              <p className="text-neutral-400">We&apos;d love to hear about your new build.</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <Link href="/contact" className="flex-shrink-0 inline-flex items-center gap-2 bg-gold-500 hover:bg-gold-400 text-black font-semibold px-7 py-3.5 rounded-lg transition-all hover:scale-105 whitespace-nowrap">
              Get in Touch
            </Link>
          </Reveal>
        </div>
